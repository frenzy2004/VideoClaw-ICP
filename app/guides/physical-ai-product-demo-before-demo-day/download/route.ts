...
export function physicalAiGuideMarkdown() {
  return [
    ...
    '',
    '## 48-hour clock',
    ...physicalAiGuide.activationClock.filter((item) => item.window === '48 hours').map((item) => `- ${item.label}: ${item.action}`),
    '',
    '## 14-day sequence',
    ...physicalAiGuide.activationClock.filter((item) => item.window === '14 days').map((item) => `- ${item.label}: ${item.action}`),
    '',
    '## Approval boundary',
    physicalAiGuide.approvalBoundary,
    '',
    '## Review prompts',
    ...physicalAiGuide.reviewPrompts.map((prompt) => `- ${prompt}`),
    '',
    '## Measurement',
