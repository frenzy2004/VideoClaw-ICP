...
const manifest=JSON.parse(await readFile(resolve(root,'inventory.json'),'utf8')) as {items:Array<{id:string;slug:string;campaign:string;articleSha256:string}>};
assert.equal(manifest.items.length,50);
await mkdir(resolve(lander,'public/media/blog'),{recursive:true});
for(const item of manifest.items) {
  await copyFile(resolve(root,'articles',item.slug+'.md'),resolve(lander,'content/articles',item.slug+'.md'));
  await copyFile(resolve(root,'media',item.slug+'.svg'),resolve(lander,'public/media/blog',item.slug+'.svg'));
}
...
console.log(JSON.stringify({nativeBase:result.nativeBase,batchCount:batch.length,combinedArticleCount:all.length,publishedCount:0,pass:true,files:(await readdir(resolve(root,'articles'))).length}));
