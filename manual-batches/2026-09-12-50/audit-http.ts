...
const inventory=JSON.parse(await readFile(resolve(root,'inventory.json'),'utf8')) as {items:Array<{slug:string}>};
const checks=[];
...
for(const item of inventory.items){
  const {data}=matter(await readFile(resolve(root,'articles',item.slug+'.md'),'utf8'));
...
assert.equal((await get('/blog/unknown-manual50-slug')).status,404);
