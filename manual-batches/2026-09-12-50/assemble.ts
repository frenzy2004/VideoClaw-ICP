...
for(const topic of topics) {
  ...
  const evidence=records.filter(row=>row.id===topic.id&&normalize(row.query??'')===normalize(topic.keyword)&&row.organicResults?.length>0)
    .sort((a,b)=>b.observedAt.localeCompare(a.observedAt))[0];
  assert(evidence,`Missing exact-term organic SERP evidence for ${topic.keyword}`);
...
assert.equal((await readdir(resolve(root,'articles'))).filter(f=>f.endsWith('.md')).length,50);
const inventory={schemaVersion:1,kind:'manual_review_batch',count:output.length,productionChanges:false,scheduledWorkerPilot:false,items:output};
await writeFile(resolve(root,'inventory.json'),JSON.stringify(inventory,null,2)+'\n');
const rows=output.map(row=>`| ${row.id} | [${row.title}](articles/${row.slug}.md) | ${row.keyword} | ${row.words} | review |`).join('\n');
await writeFile(resolve(root,'INDEX.md'),`# Fifty manual review articles\n\nTen articles per ICP. These are unpublished editorial drafts, not demand-validated or team-approved posts. All volume, difficulty and CPC values remain provider-pending.\n\n| ID | Markdown article | Primary query | Body words | Status |\n|---|---|---|---:|---|\n${rows}\n`);
console.log(JSON.stringify({stage:'assembled',articles:output.length,graphics:output.length,words:output.reduce((n,a)=>n+a.words,0),warnings:output.filter(a=>a.warnings.length).map(a=>({slug:a.slug,warnings:a.warnings}))}));
