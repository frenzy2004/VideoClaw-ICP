...
const allTopics = JSON.parse(await readFile(resolve(directory, 'topics.json'), 'utf8')) as {id:string;keyword:string}[];
if (allTopics.length !== 50 || new Set(allTopics.map(t => t.keyword)).size !== 50) throw new Error('Expected the approved fifty distinct batch topics.');
const supplementId = process.argv.find(arg=>arg.startsWith('--supplement='))?.split('=')[1];
const supplementIds = supplementId?.split(',');
const topics = supplementIds ? allTopics.filter(topic=>supplementIds.includes(topic.id)) : allTopics;
...
const output = resolve(directory, supplementId ? topics.length === 1 ? `serp-${topics[0].id}.json` : 'serp-refined-evidence.json' : 'serp-evidence.json');
