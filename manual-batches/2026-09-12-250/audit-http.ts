...
import { collectLocalLinks } from './batch-utils';
...
const cache=new Map<string,Promise<{status:number;text:string}>>();
const attribute=(tag:string,name:string)=>tag.match(new RegExp(`\\b${name}="([^"]*)"`))?.[1];
async function get(path:string){
  const url=new URL(path,origin);assert.equal(url.origin,origin,'Local links only.');url.hash='';
  const key=url.href;
  if(!cache.has(key))cache.set(key,fetch(url,{redirect:'manual'}).then(async response=>({status:response.status,text:await response.text()})));
  return cache.get(key)!;
}
const index=await get('/blog');assert.equal(index.status,200);
for(const path of collectLocalLinks(index.text))internalLinks.add(path);
for(const item of inventory.items){
  ...
  for(const path of collectLocalLinks(page.text))internalLinks.add(path);
...
for(const path of internalLinks)assert.equal((await get(path)).status,200,path);
...
const report={checkedAt:new Date().toISOString(),origin,articleCount:checks.length,mediaAssets:media.size,internalLinkCount:internalLinks.size,internalLinksAll200:true,unknownSlug404:true,download200:true,previewSitemapExcludesDrafts:true,previewRobotsDisallowAll:true,previewLlmsExcludesDrafts:true,checks};
await writeFile(resolve(root,'http-audit.json'),JSON.stringify(report,null,2)+'\n');
console.log(JSON.stringify({articles:checks.length,mediaAssets:media.size,internalLinks:internalLinks.size,pass:true}));
