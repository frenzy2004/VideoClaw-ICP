...
Latest checkpoint: [contextual source admission](CONTEXTUAL-SOURCE-ADMISSION-2026-09-11.md). A one-call live component admits the four previously rejected guide pages with exact body anchors and URL/hash attribution. A subsequent untouched backlog attempt reached the new semantic check but failed on an exact quote's wrong paragraph index; that narrow reference defect is now corrected with auditable same-page lookup, not text rewriting. State is 29 runs / 50 failures. One uninterrupted fresh article-to-native-QA proof remains outstanding; neither tests nor source admission constitute article approval.
