...
Latest checkpoint: [voice-over attempt and procedure-context correction](VOICEOVER-REFERENCE-2026-09-12.md). Automatic research, source verification, FAQ preparation, draft, critique, one repair and final independent review completed. One deterministic reference-context check stopped the fresh run before native QA. Its correction passes 2,820 tests, lint/typecheck/build and independent review. Exact saved responses now produce a review-only voice-over Markdown/SVG article passing the lander's 32 blog checks, lint and build. State retains 38 runs / 62 failures, no successful pilot. Saved-response native success is not fresh uninterrupted proof.

Previous checkpoint: [explicit body-only source review](SHOT-LIST-TRANSPORT-2026-09-12.md#explicit-body-only-source-review) records the source-input correction, its live component proof and earlier failed attempts.
...
Historical checkpoint: [bounded PAA completion and failure receipts](PAA-COMPLETION-PROOF-2026-09-10.md), with **2,530 tests plus lint/typecheck/build** passing at that checkpoint. Deep discovery can complete an initial zero-to-two-question pool; three relevant distinct questions remain mandatory before drafting. Failed attempts retain safe completed support-search receipts and count toward the ten-slot limit; malformed receipts cannot strand a lease. A separate live research-only diagnostic retrieved four pages and revealed a creation-alias duplicate, now rejected without rewriting observations. The examples worker run remains failed; no retry reset occurred.

At that historical checkpoint, the software-selection worker run entered deep discovery, then failed for insufficient relevant FAQs before OpenAI or drafting. Its support receipt and one attempted deep slot were saved correctly; state then had 24 runs / 40 failures with no pilot consumption. This was safe rejection, not a completed article.
...
Latest checkpoint: [distinct FAQ readiness](DISTINCT-FAQ-READINESS-2026-09-09.md) records two live research screens and a corrected make/create duplicate-selection bug. The fix preserves distinct qualifiers and exact observed wording; 2,170 tests, lint, typecheck and worker build pass. Neither screen established three distinct supported FAQ intents. There were zero OpenAI requests, full worker attempts, accepted articles or native article QA runs. State/history and production are unchanged; research charges were $0.06730. **The complete live article milestone remains unproven.** FAQ evidence matching needs a design review before further paid article attempts.

Previous software checkpoint: [support-search FAQ recovery](SUPPORT-FAQ-RECOVERY-2026-09-09.md) retains real questions previously discarded from support-search responses and corrects bounded cost/taxonomy matching. Follow-up review fixed a `top-down` camera-orientation false positive without permitting unsupported comparisons. That checkpoint passed 2,167 tests, lint, typecheck and build. Saved Apify results plus newly fetched page bodies yielded three supported observed FAQs and reached the first model boundary. No new paid actor or OpenAI request, accepted article, native article QA, retry reset or production change occurred. The report includes both current PR links and the separate team-access dependencies.

Latest full worker attempt: the [explainer-video live attempt](EXPLAINER-LIVE-PROOF-2026-09-09.md) collected eight organic results, four observed questions and four source bodies, then failed two comparative-FAQ evidence checks before any OpenAI request. No accepted article or native article QA resulted. All prior history is preserved; the new candidate is terminal at attempt one. The later software diagnostic does not relabel this failure as successful.

Preceding checkpoint: the [FAQ discovery and selection fix](FAQ-DISCOVERY-FIX-2026-09-09.md) searches observed questions, retrieves their supporting pages safely and selects three body-supported FAQs from a bounded relevant observation pool. A paid support-search diagnostic plus saved-data replay reached the first model boundary with all three FAQ anchors for its tutorial-video corpus. No OpenAI call, accepted article or worker retry occurred in that diagnostic. That result does not establish FAQ coverage for arbitrary new topics.

The preceding [procedure-context FAQ fix](FAQ-CONTEXT-FIX-2026-09-09.md) recognises concrete body instructions under an explicit procedure heading. Its unchanged-input replay still blocks the naming FAQ; the later automatic selection correction above is separate from that historical result.

Previous full worker attempt on 2026-09-09 Malaysia time: the user-authorized [paid tutorial-video proof](FAQ-PAID-PROOF-2026-09-09.md) collected seven organic results and four source bodies, then stopped on two FAQ-evidence findings **before any OpenAI call**. There is no accepted article or native QA result from that run. Its software/research findings led to the later diagnostic corrections above, not a retry reset. Failed history is preserved; production and scheduling remain untouched.
