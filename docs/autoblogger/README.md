...
Latest software checkpoint: [support-search FAQ recovery](SUPPORT-FAQ-RECOVERY-2026-09-09.md) retains real questions previously discarded from support-search responses and corrects bounded cost/taxonomy matching. Follow-up review fixed a `top-down` camera-orientation false positive without permitting unsupported comparisons. Fresh verification passes 2,167 tests, lint, typecheck and build. Saved Apify results plus newly fetched page bodies yielded three supported observed FAQs and reached the first model boundary. No new paid actor or OpenAI request, accepted article, native article QA, retry reset or production change occurred. **The complete live article milestone remains unproven.** The report includes both current PR links and the separate team-access dependencies.
