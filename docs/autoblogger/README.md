...
Latest checkpoint: the [FAQ discovery and selection fix](FAQ-DISCOVERY-FIX-2026-09-09.md) searches observed questions, retrieves their supporting pages safely and selects three body-supported FAQs from a bounded relevant observation pool. A paid support-search diagnostic plus saved-data replay now reaches the first model boundary with all three FAQ anchors. No OpenAI call, accepted article or worker retry occurred. **The research/preflight blocker is resolved; the complete live article milestone is still unproven.**
