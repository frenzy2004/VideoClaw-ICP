...
- [x] Write public tests first, then confirm failures. Synthetic valid original and
...
```
- [x] Strict provider/local schema: object root, schemaVersion 1,
...
- [x] Reject invalid original/policy before requests. Reuse inspectRepairDelta on
...
- [x] Run focused tests/lint and commit only assigned two files. Report red/green.

### Task 2: Normal-path integration

Files: drafting.ts, drafting.test.ts, offline-e2e.test.ts.
- [x] Prepared-context repair uses request name `videoclaw_article_repair_patch_v1`,
...
```
- [x] No fallback or second repair. On blocked return findings. On ready, existing
  content/delta checks and independent verification use assembled full draft.
- [x] Tests prove immutable bindings survive, forbidden patch blocks before verifier,
...
- [x] Full tests/lint/typecheck/build, independent review, compact checkpoint.
