...
export function parseLocalPilotArguments(argv: string[]): LocalPilotArguments {
  ...
  const nextFresh = argv.length === 7 && argv[2] === '--candidate-file' && !!argv[3]?.trim()
    && !argv[3].startsWith('--') && !argv[3].includes('\0') && argv[4] === '--approve-next-candidate-from'
    && safeRunId(argv[5]) && argv[5] !== argv[1] && argv[6] === '--execute';
...
export function reconcileLocalPilotCandidate(input: LocalPilotCandidateInput) {
  ...
  const extraApprovals = [input.approveTargetExtraAttempt, input.approveSourcePlanRetry, input.approveReviewRepairRetry, input.approveScopeAlignmentRetry, input.approveEngineeringResume, input.approveQualityRevalidation].filter(flag => flag === true).length;
  ...
  if (input.approveFreshCandidate || input.approveNextCandidateFrom !== undefined) {
    if ((input.approveFreshCandidate && input.approveNextCandidateFrom !== undefined) || input.approveRetryFrom !== undefined || input.switchTargetFrom !== undefined || input.retryTargetFrom !== undefined || extraApprovals > 0) {
    ...
    const proposedQueue = addExact(state.queuedCandidates);
    // A full queue stays intact here. The exact candidate is retained in its
    // approval and transient backlog, then reserved by the worker before paid work.
    const queuedCandidates = proposedQueue.length <= 500 ? proposedQueue : state.queuedCandidates;
    state = grantManualFreshCandidate(state, candidate, { runId: input.runId, approvedAt: input.approvedAt ?? '',
      ...(input.approveNextCandidateFrom !== undefined ? { previousFreshRunId: input.approveNextCandidateFrom } : {}),
    });
    ...
    if (input.retryTargetFrom !== undefined) {
      if (approval?.retry && !approval.retry.consumedAt) {
        if (approval.retry.priorRunId !== input.retryTargetFrom
          ...
          || (approval.retry.reason === 'manual_quality_revalidation') !== (input.approveQualityRevalidation === true)
          ...
          || (input.approveQualityRevalidation && JSON.stringify(approval.retry.evidence) !== JSON.stringify(QualityRevalidationEvidenceSchema.parse(input.qualityRevalidationEvidence)))
        ...
        ...(input.approveQualityRevalidation === true ? { qualityRevalidation: true, qualityRevalidationEvidence: input.qualityRevalidationEvidence } : {}) });
    } else if (approval) {
...
export async function readLocalEngineeringResumeEvidence(root: string, priorRunId: string, candidate: Candidate): Promise<EngineeringResumeEvidence> {
  return validateEngineeringResumeEvidence({ ...await readLocalRetryReceipts(root, priorRunId), priorRunId, candidate });
}
...
async function readLocalRetryReceipts(root: string, priorRunId: string) {
  ...
  return { reportText, auditText, implementationHash: localImplementationHash(root) };
}
...
export async function runLocalArtifactPilot(options: LocalPilotArguments & { root: string }): Promise<AutobloggerRunReport> {
  parseLocalPilotArguments(['--run-id', options.runId, ...(options.approveRetryFrom !== undefined ? ['--approve-retry-from', options.approveRetryFrom] : []),
    ...
    ...(options.approveFreshCandidate === true ? ['--approve-fresh-candidate'] : []),
    ...(options.approveNextCandidateFrom !== undefined ? ['--approve-next-candidate-from', options.approveNextCandidateFrom] : []), '--execute']);
  ...
  try {
    ...
    const qualityRevalidationEvidence = options.approveQualityRevalidation
      ? await readLocalQualityRevalidationEvidence(root, options.retryTargetFrom!, selectedCandidate) : undefined;
    const prepared = reconcileLocalPilotCandidate({ state: initial.state, backlog: await loadBacklogCandidates(root), candidate: selectedCandidate, runId: options.runId,
      ...
      ...(options.approveNextCandidateFrom !== undefined ? { approveNextCandidateFrom: options.approveNextCandidateFrom, approvedAt: new Date().toISOString() } : {}),
    });
    const freshImplementationHash = options.approveFreshCandidate || options.approveNextCandidateFrom !== undefined ? localImplementationHash(root) : undefined;
    ...
    const worker = createAutobloggerWorker({
      ...
      persistArtifact: async (artifact, report) => {
        await recheckQuality();
        return writeAutobloggerArtifacts({ ...report, artifacts: [artifact] }, outputDirectory, root);
      },
    });
