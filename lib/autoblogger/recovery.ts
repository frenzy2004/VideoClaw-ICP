...
export function hasManualFreshCandidate(
  state: PersistentWorkerState, candidateInput: Candidate, runId: string | undefined,
  mode: RunMode | undefined, nowIso: string,
): boolean {
...
export function grantManualTargetRetry(stateInput: PersistentWorkerState, candidate: Candidate,
  input: { priorRunId: string; runId: string; approvedAt: string; extraAttempt?: boolean; sourcePlanRetry?: boolean; reviewRepairRetry?: boolean; scopeAlignmentRetry?: boolean; engineeringResume?: boolean; engineeringResumeEvidence?: EngineeringResumeEvidence; qualityRevalidation?: boolean; qualityRevalidationEvidence?: QualityRevalidationEvidence }): PersistentWorkerState {
  const state = PersistentWorkerStateSchema.parse(stateInput);
  const target = state.manualTargetSwitch;
  if (state.manualFreshCandidateApproval || !target?.consumedAt || (target.retry && !target.retry.consumedAt) || state.manualPilot !== null
...
export function reserveCandidate(
  ...
  const qualityRevalidation = targetRetry && targetSwitch?.retry?.reason === 'manual_quality_revalidation';
  if ((existing?.attempts ?? 0) > MAX_CANDIDATE_ATTEMPTS && !sourcePlanRetry && !reviewRepairRetry && !scopeAlignmentRetry && !engineeringResume && !qualityRevalidation) throw new Error('Manual retry approval is consumed; retry limit is exhausted.');
  ...
  if (attempts > MAX_CANDIDATE_ATTEMPTS && !manualRetry && !extraTargetAttempt && !sourcePlanRetry && !reviewRepairRetry && !scopeAlignmentRetry && !engineeringResume && !qualityRevalidation) throw new Error('Candidate retry limit is exhausted.');
  const reserved = fresh ? { ...state, manualFreshCandidateApproval: { ...state.manualFreshCandidateApproval!, consumedAt: updatedAt } } : manualRetry ? {
...
export function markCandidateFailure(
  ...
  const consumedFresh = state.manualFreshCandidateApproval?.consumedAt
    && candidateFingerprints(state.manualFreshCandidateApproval.candidate).candidate === candidateFingerprints(candidate).candidate;
  const status: CandidateDecision['status'] = reconciliation
    ? 'manual_attention'
    : retryable && !consumedSwitch && !consumedFresh && attempts < MAX_CANDIDATE_ATTEMPTS ? 'retryable' : 'terminal';
...
export function deferCandidate(
  ...
  if ((existing?.attempts ?? 0) > MAX_CANDIDATE_ATTEMPTS
    || (state.manualFreshCandidateApproval?.consumedAt && candidateFingerprints(state.manualFreshCandidateApproval.candidate).candidate === candidateFingerprints(candidate).candidate)
...
export function recoverExpiredReservations(state: PersistentWorkerState, nowIso: string): PersistentWorkerState {
  ...
  const decisions = Object.fromEntries(Object.entries(state.decisions).map(([fingerprint, decision]) => {
    ...
    const exhausted = decision.attempts >= MAX_CANDIDATE_ATTEMPTS
      || !!(state.manualFreshCandidateApproval?.consumedAt && candidateFingerprints(state.manualFreshCandidateApproval.candidate).candidate === fingerprint);
    return [fingerprint, {
      ...decision,
      status: exhausted ? 'terminal' : 'retryable',
      reason: exhausted ? 'lease_expired_retry_limit' : 'lease_expired',
      updatedAt: nowIso,
      leaseExpiresAt: null,
